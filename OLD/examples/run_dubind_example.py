...
def run_training(training_set,testing_set,json_params,mode,k=0):
    ...
    with tf.Session(graph = model_graph) as sess:
        ...
        if (mode=='train' or mode =='validate'):
            ...
            eval_output_file = json_params.eval_predictions_file
...
def write_predictions(sess,
    ...
    final_write_buffer = []
    final_buffer_index = num_buffers*buffer_size #This index is the first index of the final buffer

    ##DEBUG
    #eval_indices = range(final_buffer_index,eval_indexer.num_elements)
    ##DEBUG
        ...
        for i in eval_indices:
            ...
            if ma_eval_indexer[i]==False:
                ...
                if i >= final_buffer_index:
                    final_write_buffer.append(''.join([contig,'\t',start,'\t',end,'\t',prob,'\n']))
                else:
                    write_buffer[write_index]=''.join([contig,'\t',start,'\t',end,'\t',prob,'\n'])
            ...
            write_index += 1
            if ((i+1) % buffer_size)==0 and i>0 and i < final_buffer_index:
                ...
                print "Wrote",i+1,"predictions to", output_file,"in",tf,"s"
                print "Write time took",tfwrite,"s"
                if i == final_buffer_index:
                    "Final buffer index reached"
            elif i == (eval_indexer.num_elements -1) and i >= final_buffer_index:
                print "Writing final buffer which has len", len(final_write_buffer)
                f.writelines(final_write_buffer)
